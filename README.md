...
4. macOS 上挂一个每日静默版本检查的 launchd agent。
...
v3.0.2 之前 `inspire update` 在 `uv tool` 装的环境上会因为 detector bug 拒绝自动升级。修好的版本需要另外灌进来一次：

**重跑 install.sh**（等价于「原地强制覆盖」）：
...
不常驻 SII 的科研人员通常需要让本机代理同时转发公网和 `*.sii.edu.cn` 流量。仓库提供一份**可选**的 Clash Verge `7897` mixed-port 分流模板，见 [references/proxy-setup.md](references/proxy-setup.md)；但 CLI 本身不绑定 7897。代理地址通过 `inspire account add` 写入账号配置，并可用 `inspire config show --compact` 核对。
...
- [references/compute-workloads.md](references/compute-workloads.md) — GPU job、CPU HPC、Ray 的适用边界、调度语义和示例。
- [references/workflows.md](references/workflows.md) — CPU 准备、数据处理、分布式训练三阶段项目流程。
- [references/proxy-setup.md](references/proxy-setup.md) — Clash Verge 7897 分流配置和账号级代理衔接。
