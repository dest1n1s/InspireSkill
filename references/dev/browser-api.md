...
网页端部署表单已实测确认：当前 CLI Agent 合同覆盖“自定义部署”（`inference_serving_type:"CUSTOM"`）。自定义部署使用注册模型、镜像 `mirror_id`、运行命令、端口、优先级、资源规格、副本数、单副本实例数和共享内存。`resources specs --usage serving` 必须查 `SCHEDULE_CONFIG_TYPE_SERVE`，不是训练任务的 `SCHEDULE_CONFIG_TYPE_TRAIN`。在 `H200-2号机房` 实测到的 serving 规格是 `1,18,200`、`2,36,400`、`4,72,900`、`8,168,1800`。
